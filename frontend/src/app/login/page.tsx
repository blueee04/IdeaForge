  ...
  const handleOAuth = async (provider: "google" | "github") => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          redirectTo: `${window.location.origin}/dashboard/seeker` // Default redirect
        }
      });
      if (error) throw error;
    } catch (err: any) {
      alert(err.message || `An error occurred connecting to ${provider}.`);
      setLoading(false);
    }
  };
  return (
      ...
      <div className="auth-container">
        <div className="auth-card">
          ...
          <form className="auth-form" onSubmit={handleSubmit}>
            {/* OAuth buttons */}
            <button type="button" className="btn-oauth" onClick={() => handleOAuth("google")} disabled={loading}>
            ...
            <button type="button" className="btn-oauth" onClick={() => handleOAuth("github")} disabled={loading}>
